...
The Sourcegraph Sales Workbench web UI provides a modern interface for executing sales agents with full MCP (Model Context Protocol) access. This document explains how the system is architected to ensure agents have access to Salesforce, Gong, and Notion data.
...
```bash
npx tsx src/agents/refreshData.ts "Acme Corp"
```
