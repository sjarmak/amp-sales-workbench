...
async function loadTestAccountData(): Promise<SnapshotData> {
  const accountDir = path.join(__dirname, '..', 'data', 'accounts', 'test-account');
  const snapshotsDir = path.join(accountDir, 'snapshots');
  
  console.log('📂 Loading TestAccount snapshot data...');
...
async function testNotionMirror() {
  ...
  // Load test data
  const snapshot = await loadTestAccountData();
  ...
  const payload = {
    ...
    nextActions: snapshot.nextActions || [
      ...
      'Schedule technical review with engineering lead'
    ]
  };
...
async function testAccountSpecificEndpoint() {
  console.log('\n🔄 Testing POST /api/accounts/test-account/notion/mirror...');
  
  try {
    const response = await fetch(`${API_BASE}/api/accounts/test-account/notion/mirror`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function main() {
  try {
    ...
    if (test1 && test2) {
      ...
      console.log('\n📝 What was tested:');
      console.log('   ✓ Loading TestAccount account data from snapshots');
      ...
      console.log('\n🔗 Next steps:');
      console.log('   → Check the "TestAccount (Workbench Test)" page in Notion');
