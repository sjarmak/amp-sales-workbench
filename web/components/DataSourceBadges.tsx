...
export function DataSourceBadges({ accountSlug, capabilities, refreshTrigger = 0 }: DataSourceBadgesProps) {
  ...
  const probeAndAutoRefresh = async () => {
    try {
      ...
      if (probeResults.sourcegraph?.staleOnSource) staleSources.push('sourcegraph')
  ...
  const capitalizeSource = (source: string) => {
    ...
    if (source === 'sourcegraph') return 'Sourcegraph Docs'
  ...
  const openAllSourcesDetails = () => {
    ...
    Promise.all([
      ...
      capabilities.sourcegraph ? fetch(`${API_URL}/accounts/${accountSlug}/sources/sourcegraph`).then(r => r.json()) : null,
    ]).then(([sf, gong, notion, sg]) => {
      setSourceData({ salesforce: sf, gong, notion, sourcegraph: sg })
      setLoading(false)
    }).catch(() => {
      setSourceData({ error: 'Failed to load data' })
      setLoading(false)
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  ...
                  title={source === 'sourcegraph' ? 'Refresh Sourcegraph data' : 'Refresh data source (requires MCP configuration)'}
                >
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {selectedSource === 'sourcegraph' && sourceData && (
                <>
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-2xl font-bold">
                        {sourceData.summary?.stats?.totalHeadings || 0}
                      </div>
                    </div>
                    <div className="space-y-1">
                  ...
                  {sourceData.summary && (
                    <div className="space-y-3">
                      ...
                      {sourceData.summary.sections.docs?.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-medium text-blue-600">Docs ({sourceData.summary.sections.docs.length} sections)</h4>
                          <div className="border rounded overflow-hidden">
                            <table className="w-full text-xs">
                              ...
                              <tbody className="divide-y">
                                {sourceData.summary.sections.docs.slice(0, 10).map((section: any, i: number) => (
                      ...
                      {sourceData.summary.sections.blog?.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-medium text-green-600">Blog ({sourceData.summary.sections.blog.length} sections)</h4>
                          <div className="border rounded overflow-hidden">
                            <table className="w-full text-xs">
                              <thead className="bg-gray-50">
                                <tr>
                              ...
                              <tbody className="divide-y">
                                {sourceData.summary.sections.blog.slice(0, 10).map((section: any, i: number) => (
                                  <tr key={i} className="hover:bg-gray-50">
                                    <td className="px-3 py-2 font-medium whitespace-nowrap">{section.heading}</td>
                                    <td className="px-3 py-2 text-muted-foreground">
                                      {section.content.substring(0, 100)}...
                  ...
                  {(sourceData.content?.docs || sourceData.content?.blog) && (
                    <div className="space-y-3">
                      <h3 className="font-semibold">Full Content (for Context)</h3>
                      <div className="text-xs text-muted-foreground">
                        This content is stored and available for agents to use in analysis and document creation.
                      </div>

                      {sourceData.content.docs && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-medium text-blue-600">Docs</h4>
                      ...
                      {sourceData.content.blog && (
                        <div className="space-y-2">
                          <h4 className="text-sm font-medium text-green-600">Blog</h4>
                          <div className="border rounded p-3 bg-gray-50 max-h-60 overflow-y-auto">
                            <pre className="text-xs whitespace-pre-wrap font-mono">
                              {sourceData.content.blog}
                            </pre>
