 ...
 * 
 * Defines the sales lifecycle stages with their properties,
 * quick actions, and Salesforce stage mappings.
 */

import type { LifecycleStageId, AgentId, ProductId } from '../agentTypes.js'
...
export interface LifecycleStage {
	...
	sfStages: string[] // Matching Salesforce opportunity stages
}
...
/**
 * Get lifecycle stage from Salesforce stage name.
 */
export function getStageFromSalesforce(sfStage: string): LifecycleStageId {
	for (const stage of LIFECYCLE_STAGES) {
		if (
			stage.sfStages.some(
				(s) => s.toLowerCase() === sfStage.toLowerCase()
			)
		) {
			return stage.id
		}
 ...
 */
export function getStageConfig(id: LifecycleStageId): LifecycleStage {
 ...
 */
export function getNextStage(id: LifecycleStageId): LifecycleStage | undefined {
	const current = getStageConfig(id)
	return LIFECYCLE_STAGES.find((s) => s.order === current.order + 1)
}

/**
 * Get the previous stage in the lifecycle.
 */
export function getPreviousStage(id: LifecycleStageId): LifecycleStage | undefined {
	const current = getStageConfig(id)
	return LIFECYCLE_STAGES.find((s) => s.order === current.order - 1)
}
 ...
 */
export function getOrderedStages(): LifecycleStage[] {
	return [...LIFECYCLE_STAGES].sort((a, b) => a.order - b.order)
}
...
/**
 * Get Tailwind color class for a stage.
 */
export function getStageColorClass(id: LifecycleStageId): string {
	const stage = getStageConfig(id)
...
/**
 * Get border color class for a stage.
 */
