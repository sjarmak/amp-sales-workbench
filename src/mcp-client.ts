...
export async function callSalesforceTool(toolName: string, args: any): Promise<any> {
  ...
  try {
    ...
    console.log(`[mcp-client] SF Result type:`, content[0]?.type, 'length:', content.length);
    if (content[0]?.type === 'text') {
      console.log(`[mcp-client] SF Raw text (first 300 chars):`, content[0].text.substring(0, 300));
    }
    
    // Check if MCP returned an error in the content
    if (content[0]?.type === 'text' && content[0].text) {
      const text = content[0].text;
      // If text looks like a raw error (not JSON), log it prominently
      if (!text.trim().startsWith('{') && !text.trim().startsWith('[')) {
        console.error(`[mcp-client] ⚠️ SF MCP returned non-JSON response:`, text.substring(0, 500));
      }
    }
    
    return content;
  } catch (error: any) {
    console.error(`[mcp-client] SF Tool call failed:`, error.message || error);
    console.error(`[mcp-client] SF Error stack:`, error.stack);
    throw error;
  }
}
