...
```typescript
{
  calls: [
    {
      id: "123",
      title: "AcmeCorp<>Sourcegraph: Monthly Cadence",
      scheduled: "2025-10-20T21:00:00Z",
      duration: 2141,
      companyNames: ["acmecorp"],  // ← Enriched!
      ...
    }
  ],
...
1. Run any agent that queries Gong (e.g., for AcmeCorp)
...
const calls = await manager.getCallsForAccount('AcmeCorp', {
...
npm run gong:stats -- --account="AcmeCorp"
...
|---------|---------|-----------|
| `Company <> Sourcegraph` | "AcmeCorp<>Sourcegraph: Monthly" | `acmecorp` |
| `Sourcegraph x Company` | "Sourcegraph x TechInc" | `techinc` |
| `Company / Sourcegraph` | "GlobalCo / Sourcegraph" | `globalco` |
| `Company + Sourcegraph` | "MegaCorp + Sourcegraph" | `megacorp` |
