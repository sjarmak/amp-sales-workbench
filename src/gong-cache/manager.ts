...
export class GongCacheManager {
	...
	async sync(): Promise<GongCacheSyncResult> {
		...
		if (daysDiff > 14) {
			...
			try {
				...
				if (!resultContent || !Array.isArray(resultContent) || !resultContent[0]) {
					console.error('[Gong Cache] Invalid response structure from list_calls:', resultContent)
					throw new Error('Invalid response structure from Gong MCP')
				}
				...
				try {
					...
					console.error('[Gong Cache] Failed to parse response:', rawText.substring(0, 300))
					throw new Error(`Failed to parse Gong response: ${rawText.substring(0, 100)}`)
				}
				
				rawCalls = result.calls || []
			} catch (error: any) {
				console.error('[Gong Cache] Sync failed:', error.message || error)
	 ...
	 * 
	 * @param accountName - Company name or search term to match
	 * @param options - Additional filtering options
	 */
	async getCallsForAccount(accountName: string, options?: {
		maxResults?: number
		since?: Date
		domain?: string // Optional: filter by participant email domain (e.g., "acme.com")
		...
		let filtered = this.index!.calls.filter(call => {
			// Match if search term is in extracted names or in title
			const titleMatch = call.title.toLowerCase().includes(searchName)
			// companyNames are already lowercase in cache (see enrichment.ts line 57/70)
