...
import type { AgentName } from './agents/agent-runner.js'
...
function parseArgs(): CliArgs {
	...
	const findArg = (flag: string) => args.find((_, i) => args[i - 1] === flag)
	const domain = findArg('--domain')
	const salesforceId = findArg('--sfid')
	const apply = args.includes('--apply')
	const forceResearch = args.includes('--force-research')
	const agent = findArg('--agent') as AgentName | undefined
	const callId = findArg('--call-id')
	const date = findArg('--date')
	const methodology = findArg('--methodology') as 'MEDDIC' | 'BANT' | 'SPICED' | undefined
