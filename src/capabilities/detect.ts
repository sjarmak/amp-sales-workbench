...
async function testGong(): Promise<boolean> {
	try {
		// Check GONG_SOURCE to determine test approach
		const gongSource = process.env.GONG_SOURCE?.toLowerCase() || 'parquet';
		
		// If using parquet, just check if lakehouse exists
		if (gongSource === 'parquet') {
			const { homedir } = await import('os');
			const { join } = await import('path');
			const parquetPath = join(homedir(), 'gong_data', 'data', 'bronze', 'calls.parquet');
