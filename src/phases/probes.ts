...
/**
 * Probe Sourcegraph pages for Last-Modified headers
 */
export async function probeSourcegraph(
	_accountDataDir: string,
	meta: SourcesMeta
): Promise<ProbeResult> {
	try {
		const urls = [
			{ key: 'docs', url: 'https://sourcegraph.com/docs' },
			{ key: 'blog', url: 'https://sourcegraph.com/blog' },
		]

		const promises = urls.map(u => fetchLastModified(u.url))
		...
		urls.forEach((item, index) => {
			const lastModified = results[index]
			if (lastModified) {
				// @ts-ignore
				const localFetched = meta.sources.sourcegraph?.pages?.[item.key]?.lastFetchedAt
				if (localFetched && new Date(lastModified) > new Date(localFetched)) {
					staleOnSource = true
					reasons.push(`Sourcegraph ${item.key} modified at ${lastModified}`)
				}
			}
		})
