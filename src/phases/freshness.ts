...
const TTL = {
	...
	sourcegraph: {
		docs: 24 * 60 * 60 * 1000, // 24 hours
		blog: 24 * 60 * 60 * 1000, // 24 hours
	},
}
...
export interface SourcegraphPage {
	url: string
	etag?: string
	lastModified?: string
	hash?: string
...
export interface SourcegraphDocsMetadata extends SourceMetadata {
	pages?: {
		docs?: SourcegraphPage
		blog?: SourcegraphPage
		codeSearch?: SourcegraphPage
		batchChanges?: SourcegraphPage
		codeInsights?: SourcegraphPage
		mcp?: SourcegraphPage
	}
...
export interface SourcesMeta {
	...
	sources: {
		...
		sourcegraph?: SourcegraphDocsMetadata
	}
}
...
export function computeStaleness(
	...
	// Sourcegraph staleness
	const sgMeta = meta.sources.sourcegraph
	const sgResult: StalenessResult = {
		any: false,
		reasons: [],
	}
	...
	if (!sgMeta || !sgMeta.pages) {
		sgResult.any = true
		sgResult.reasons.push('No Sourcegraph data found')
	} else {
		// If we just probed/synced recently (within last 5 minutes), consider fresh
		const justProbed = sgMeta.lastProbedAt && (nowMs - new Date(sgMeta.lastProbedAt).getTime()) < 5 * 60 * 1000
		
		const { docs, blog } = sgMeta.pages

		// Check docs page
		if (!docs?.lastFetchedAt) {
			sgResult.any = true
			sgResult.reasons.push('Sourcegraph docs missing')
		} else {
			const age = nowMs - new Date(docs.lastFetchedAt).getTime()
			if (age > TTL.sourcegraph.docs && !justProbed) {
				sgResult.any = true
				sgResult.reasons.push(`Sourcegraph docs stale (${Math.floor(age / (60 * 60 * 1000))}h old)`)
			}
		}

		// Check blog page
		if (!blog?.lastFetchedAt) {
			sgResult.any = true
			sgResult.reasons.push('Sourcegraph blog missing')
		} else {
			const age = nowMs - new Date(blog.lastFetchedAt).getTime()
			if (age > TTL.sourcegraph.blog && !justProbed) {
				sgResult.any = true
				sgResult.reasons.push(`Sourcegraph blog stale (${Math.floor(age / (24 * 60 * 60 * 1000))}d old)`)
			}
		}
	}
...
export function updateSourcegraphDocsCheckpoint(
	meta: SourcesMeta,
	update: Partial<SourcegraphDocsMetadata>
): void {
	if (!meta.sources.sourcegraph) {
		meta.sources.sourcegraph = {}
	}
	Object.assign(meta.sources.sourcegraph, update)
	meta.sources.sourcegraph.status = 'fresh'
}
